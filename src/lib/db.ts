...
export function getTodayETDate(): Date {
...
export async function archivePastDailyLocations() {
  try {
    const todayDate = getTodayETDate();

    // 1. Mark any location that was in a daily queue with scheduledDate < todayDate as archived
    ...
    // 2. Remove previous (past) locations from the daily queue
    ...
    console.error("Failed to dynamically archive past daily locations and clean queue:", error);
  }
}
